...
def build_assistant_message(agent, assistant_message, finish_reason: str) -> dict:
    ...
    if assistant_tool_calls:
        ...
        for tool_call in assistant_tool_calls:
            ...
            _fn_arguments = tool_call.function.arguments
            if isinstance(_fn_arguments, str) and (
                "<experience-memory-context" in _fn_arguments.lower()
                or "<memory-context" in _fn_arguments.lower()
            ):
                # The model sometimes echoes injected hidden context fences
                # into tool-call arguments. Strip them before the call enters
                # replayable history; the executor sanitizes the live string
                # the same way, so history and dispatch stay consistent.
                from agent.memory_manager import sanitize_context
                _fn_arguments = sanitize_context(_fn_arguments)
