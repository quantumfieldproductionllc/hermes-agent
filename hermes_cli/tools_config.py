...
CONFIGURABLE_TOOLSETS = [
    ...
    ("moa",             "🧠 Mixture of Agents",         "/moa virtual provider mode"),
    ("tts",             "🔊 Text-to-Speech",            "text_to_speech"),
    ("stt",             "🎙️ Speech-to-Text",           "voice transcription (gateway voice messages + voice mode)"),
    ("skills",          "📚 Skills",                    "list, view, manage"),
    ("todo",            "📋 Task Planning",             "todo"),
    ("memory",          "💾 Memory",                    "persistent memory across sessions"),
    ("experience_memory", "🧠 Experience Memory",       "local SQLite experience memory"),
...
def _get_platform_tools(
    ...
    *,
    ...
    def _configured_toolset_tools(ts_key: str) -> Set[str]:
        """Tools used for config reverse-mapping from platform composites.

        ``resolve_toolset()`` merges live registry additions into static
        toolsets. That is correct for runtime schema resolution, but config
        reverse-mapping should not become order-dependent on which tool modules
        have already registered optional extras. A platform composite that
        contains the static terminal tools should still resolve the ``terminal``
        toolset even if the desktop-only ``read_terminal`` tool has been
        registered in this process.
        """
        ts_def = TOOLSETS.get(ts_key) or {}
        if not ts_def.get("includes"):
            static_tools = set(ts_def.get("tools") or [])
            if static_tools:
                return static_tools
        return set(resolve_toolset(ts_key))
    ...
    has_explicit_config = any(
        ts in explicit_known_keys and _toolset_allowed_by_restricted_default(ts, platform)
