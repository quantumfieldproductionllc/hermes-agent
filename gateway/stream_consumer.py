                ...
                if self._edit_supported:
                    ...
                    if result.success:
                        ...
                        if (
                            ...
                            self._final_content_delivered = True
                            # On REQUIRES_EDIT_FINALIZE platforms (Telegram), the failed
                            # finalize edit is not cosmetic: raw markdown remains
                            # visible. Heal its formatting after the flood penalty.
                            if self._is_flood_error(result):
                                _ra = getattr(result, "retry_after", None)
                                if _ra:
                                    self._last_flood_retry_after = float(_ra)
                                self._schedule_finalize_format_retry(text)
                            # ``text`` is already cleaned/fence-closed and equals
                            # the visible prefix. Record it even on split turns so
                            # the gateway does not re-send an already-visible reply.
