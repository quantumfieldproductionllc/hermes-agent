...
# ---------------------------------------------------------------------------
# Local feature tests: Telegram bot command menu curation (amelia-assistant).
# Restored after upstream's test-suite rewrite dropped coverage for the
# locally-built menu curation / priority / plugin enumeration behavior.
# ---------------------------------------------------------------------------

class TestLocalTelegramMenuCuration:
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch hermes_cli.plugins.get_plugin_commands() to a fixed dict."""
        from hermes_cli import plugins as _plugins_mod
